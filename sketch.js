...
var golimg, himg1,ghea=200,h1hea=130,dunn1=0,destroy=1,cheat1=0,cheat2=0,cheat3=0,cheat4=0,cheat5=0,simg,ability,cheat=1,d1img,d2img,ceat=1,h3hea=130,d2hea=250, cat=1,ct=1,t=1,h4hea=130,h5hea=130,cd=0,h=0,e=0,l=0,p=0,s=0;
var eimg,entity,bb,b,end=0,tb,ehea=300,bye, mapquare, mapquare2, fquare, lquare, cquare;
function preload() {
...
function setup() {
    ...
    mapquare=createSprite(100,200,20,20)
    mapquare.color="green";
    mapquare2=createSprite(100,150,20,20)
    fquare=createSprite(75,175,20,20)
    lquare=createSprite(50,150,20,20)
    cquare=createSprite(50,200,20,20)
    mapquare.color="red";
    fill("gray");
...
function draw() {
    ...
    if(frameCount<200) {
        ...
        text("space:attack e:fireball p:level up l:comet m:map h:hide tap the villagers and fight for your life.",50,300)
        text("(Don't click on the finalsquare until you finished the dungeons. Otherwise this game is prone to lag. You earn some squares later.)",50,400)
        pop();
        mapquare.visible=false;
        mapquare2.visible=false;
        fquare.visible=false;
        lquare.visible=false;
        cquare.visible=false;
    }
    else{
        end=5;
        mapquare.visible=true;
        mapquare2.visible=true;
        fquare.visible=true;
        lquare.visible=true;
        cquare.visible=true;
    }
...
if(end===5){
    ...
    if (keyDown("p") && xp>=100 || mousePressedOver(lquare) && xp>=100) {
        newP=1;
    }
    if (keyDown("up")) {
    ...
    if (keyDown("m") || mousePressedOver(mapquare)) {
        map.visible=true
    ...
    } else if (keyDown("h") || mousePressedOver(mapquare2)) {
        map.visible=false
        kingdom.visible=false
        forest.visible=false
        vil1.visible=false
    ...
    if (keyDown("e") && cooldown>=100 || mousePressedOver(fquare) && cooldown>=100) {
      cooldown=0;
    ...
    if (newP===1) {
        ...
        if (keyDown("l") && flame>=10 || mousePressedOver(cquare) && flame>=10) {
    ...
    if (b===1) {
        ...
        text("Boss Health "+Math.round(ehea),1400,100)
